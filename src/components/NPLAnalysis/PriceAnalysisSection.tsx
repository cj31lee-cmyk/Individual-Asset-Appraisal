...
export function PriceAnalysisSection({ data, purchaseInfo, onChange }: Props) {
  const update = (key: keyof PriceAnalysisInfo, value: number) => {
    ...
    // 최종매입가 = 회수예상가 - (회수예상가 × 할인율)
    next.finalPurchasePrice = next.purchaseMinusSenior - next.loanPurchasePrice;
    ...
    <Card>
      ...
      <CardContent className="space-y-3">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
          {readonlyField("최종매입가 (회수예상가-할인액)", data.finalPurchasePrice, true)}
        </div>
      </CardContent>
    </Card>
  );
